...
// Dependencies: layout.js (must be loaded before this)

// Detect browser WebP support
let supportsWebP = false;

// Method 1: Detect using canvas
...
// Method 2: Detect using image (fallback)
...
// Get file extension (based on WebP support + layout config's forcePng)
...
async function loadMemo() {
  ...
  try {
    ...
    memoContent.innerHTML = '<div id="memo-placeholder">Loading failed</div>';
  }
}
...
const STATES = {
  ...
  writing: { name: 'Organize Documents', area: 'writing' },
  researching: { name: 'Search Information', area: 'researching' },
  executing: { name: 'Execute Task', area: 'writing' },
  syncing: { name: 'Sync Backup', area: 'writing' },
  error: { name: 'Error', area: 'error' }
};

const BUBBLE_TEXTS = {
  idle: [
    'Standby: ears perked up',
    'I\'m here, ready to go',
    'Let me tidy up the desktop first',
    'Whew — giving my brain a break',
    'Productive elegance today',
    'Patience makes for precision',
    'Coffee\'s still hot, creativity flows',
    'I\'m buffing you up in the background',
    'Status: peace of mind / recharging',
    'Kitten says: slow and steady wins'
  ],
  writing: [
    'Entering focus mode: do not disturb',
    'First, run through the critical path',
    'I turn complexity into simplicity',
    'Lock those bugs in their cage',
    'Halfway done, let me save',
  ...
  researching: [
    'I\'m uncovering the evidence chain',
    ...
    'Pinpoint first, then optimize',
    'Don\'t rush, let me draw the cause map'
  ],
  executing: [
    ...
    'Minimum viable first, then polish'
  ],
  syncing: [
    'Syncing: locking today in the cloud',
    'Backup isn\'t ritual, it\'s security',
    'Writing… don\'t lose power',
    'Timestamp the changes',
    'Cloud alignment: click',
    'Don\'t touch anything before sync completes',
    'Save future-me from disaster',
    'One more backup, one less regret'
  ],
  error: [
    'Alarm sounding: don\'t panic',
    'I smell a bug',
    ...
    'I\'m on it: pinpointing root cause now',
    'Don\'t worry, I\'ve seen this before',
    'Alerting: let the problem reveal itself'
  ],
  cat: [
    'Meow~',
    'Purr purr…',
    ...
    'I\'m this office\'s lucky charm',
    'Stretch time',
    'Is my food bowl ready?',
    'Purrrr',
...
// breakroom / writing / error area agent distribution positions (offset when multiple agents)
...
function preload() {
  ...
  // Read total asset count from LAYOUT (avoid magic number)
  ...
  // New desk: force PNG (transparent)
...
function create() {
  ...
  const plaqueText = game.add.text(plaqueX, plaqueY, 'Star\'s Pixel Office', {
  ...
  // === New desk (from LAYOUT, force transparent PNG) ===
  ...
  // Test: let Nika simulate walking around
...
function renderAgent(agent) {
  ...
  // Opacity (become semi-transparent when offline/pending/rejected)
  let alpha = 1;
  ...
  if (!agents[agentId]) {
    ...
    container.setDepth(1200 + (isMain ? 100 : 0)); // Put on top!

    // Pixel character: use star icon, more visible
